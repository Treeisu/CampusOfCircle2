...
<div class="main">
	...
	<!--=====右边+中间=====-->
	<div id="rightUser">
		<div><h3 class="text-center text-warning" style="margin-top: 30px;">您的粉丝</h3></div>
		<c:if test="${fanUsers.size()>0}">
		<c:forEach var="fanUser" items="${fanUsers}">		
	    <c:if test="${!empty fanUser.fan}"><!-- 这里是粉丝 -->
	    	<div class="media pull-left" style="width:400px;margin-top: 20px;margin-left:5px;">
		    <a class="pull-left" href="#"><img class="media-object" src="<c:out value="${fanUser.userImage}"/>" style="width: 40px;height: 40px;"></a>
		    <div class="media-body">
		    	...
		    	<h5 class="media-heading text-primary " style="width:300px">
		    		...
		    		<p class="fanId" style="display: none;"><c:out value="${fanUser.fan.id}"/></p>
		    		<c:if test="${fanUser.addState==0}"><a class="btn btn-danger btn-xs addATT" style="margin-left: 10px;margin-top: 5px;">关注</a></c:if>
		    		<c:if test="${fanUser.addState==1}"><a class="btn btn-danger btn-xs resetGroup2" style="margin-left: 10px;margin-top: 5px;">分组</a></c:if>
		    	</h5>
		    	<c:if test="${empty fanUser.userSchool}"><h5 class="media-heading text-warning"><i class="icon icon-school"></i>&nbsp;&nbsp;该用户未设置学校</h5></c:if>		    	
		    	<c:if test="${!empty fanUser.userSchool}"><h5 class="media-heading text-warning"><i class="icon icon-school"></i>&nbsp;&nbsp;<c:out value="${fanUser.userSchool}"/></h5></c:if>		    	
		    	<c:if test="${!empty fanUser.userDescription}"><div class="text-muted" style="width: 300px;margin-top: 10px;"><c:out value="${fanUser.userDescription}"/></div></c:if>		   	
		    	<c:if test="${empty fanUser.userDescription}"><div class="text-muted" style="width: 300px;margin-top: 10px;">该用户很懒，什么也没留下。</div></c:if>
		    	<ul class="list-group" style="width: 300px;margin-left: 0px;">
		    		<li class="list-group-item">关注<span class="text-info">(<c:out value="${fanUser.userAdviceNum.attentionNum}"/>)</span></li>
		    		<li class="list-group-item">粉丝<span class="text-info">(<c:out value="${fanUser.userAdviceNum.fansNum}"/>)</span></li>
		    		<li class="list-group-item">动态<span class="text-info">(<c:out value="${fanUser.userAdviceNum.wbNum}"/>)</span></li>
		    	</ul>		    	
		    </div>
	    </div>
	    </c:if>
	    ...
	    <c:if test="${ empty fanUsers}">
	    	<div style="margin-top:40px;border: dashed;">
	    		<h3 class="text-center">您还没有粉丝！</h3>
	    	</div>
	    </c:if>
	</div>
	<!--=====右中=====-->
...
<footer>
	...
	<script type="text/javascript" src="js/Fans.js"></script>
