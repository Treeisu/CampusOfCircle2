...
<body onload="initPage();">
	<div style="margin-left: 100px; margin-top: 100px;">
		...
		<br>
		<!-- 后台返回结果为空 -->
		<c:if test="${fn:length(result.listShow) eq 0 }">
			<span>查询的结果不存在</span>
		</c:if>
		
		<!-- 后台返回结果不为空 -->
		<c:if test="${fn:length(result.listShow) gt 0 }">
			<table border="1px" cellspacing="0px"style="border-collapse: collapse">
				<thead>
					...
					<c:forEach items="${result.listShow }" var="student">
						<tr>
							<td><c:out value="${student.stuName }"></c:out></td>
							<td><c:if test="${ student.gender eq 1}">男</c:if><c:if test="${ student.gender eq 2}">女</c:if></td>
							<td><c:out value="${student.age }"></c:out></td>
							<td><c:out value="${student.address }"></c:out></td>
						</tr>
					</c:forEach>
			</table>
			<br> 共${result.totalNumber}条记录共${result.totalPage }页&nbsp;&nbsp;当前第${result.currentPage}页&nbsp;&nbsp;
